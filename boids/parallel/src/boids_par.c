...
void update_boids(Boids* in, Boids* out) {
	...
	next_vy += (float)(boid_y < BORDER_MARGIN) * TURN_FACTOR;
	next_vy -= (float)(boid_y > SCREEN_HEIGHT - BORDER_MARGIN) * TURN_FACTOR;
	next_vx += (float)(boid_x < BORDER_MARGIN) * TURN_FACTOR;
	next_vx -= (float)(boid_x > SCREEN_WIDTH - BORDER_MARGIN) * TURN_FACTOR;
...
void update_boids_grid(Boids* in, Boids* out, Cell (*grid)[GRID_COLS]) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int dr = -1; dr <= 1; dr++) {
            ...
            for (int dc = -1; dc <= 1; dc++) {
                ...
                for (int k = 0; k < limit; k++) {
                    ...
                    float mask_visual = (float)(dist_sq < VISUAL_RANGE_SQ);
                    float mask_protected = (float)(dist_sq < PROTECTED_RANGE_SQ);
                    float mask_cohesion = mask_visual * (1.0f - mask_protected);

                    close_dx += dx * mask_protected;
                    close_dy += dy * mask_protected;
                    xpos_avg += in->x[j] * mask_cohesion;
                    ypos_avg += in->y[j] * mask_cohesion;
                    xvel_avg += in->vx[j] * mask_cohesion;
                    yvel_avg += in->vy[j] * mask_cohesion;
                    neighboring_boids += (int)mask_cohesion;
                }
            }
        }
        ...
        if (neighboring_boids > 0) {
            float inv = 1.0f / (float)neighboring_boids;
            xpos_avg *= inv;
            ypos_avg *= inv;
            xvel_avg *= inv;
            yvel_avg *= inv;
        ...
        next_vy += (float)(boid_y < margin) * TURN_FACTOR;
        next_vy -= (float)(boid_y > SCREEN_HEIGHT - margin) * TURN_FACTOR;
        next_vx += (float)(boid_x < margin) * TURN_FACTOR;
        next_vx -= (float)(boid_x > SCREEN_WIDTH - margin) * TURN_FACTOR;

        float speed_sq = next_vx * next_vx + next_vy * next_vy;
        float speed = sqrtf(speed_sq) + 1e-9f; // evita divisione per zero
        float desired_speed = fmaxf(BOID_MIN_SPEED, fminf(BOID_MAX_SPEED, speed));
        float scale = desired_speed / speed;
...
// --- Spatial Sorting (Counting Sort Lineare) ---
void sort_flock(Boids* in, Boids* out) {
    int n = in->n;
    int num_cells = GRID_ROWS * GRID_COLS;
    
    int* counts = (int*)calloc(num_cells, sizeof(int));
    int* offsets = (int*)calloc(num_cells, sizeof(int));

    for (int i = 0; i < n; i++) {
        int col = (int)(in->x[i] / CELL_SIZE);
        int row = (int)(in->y[i] / CELL_SIZE);
